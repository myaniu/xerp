...
public class ImageVerification {
	...
	public BufferedImage creatImage() {
		...
		// 在内存中创建图象
		int width = 10 * IMAGE_VERIFICATION_LENGTH + 30, height = 30;
		...
		for (int i = 0; i < IMAGE_VERIFICATION_LENGTH; i++) {
			...
			String rand = String.valueOf(codeSeq[random.nextInt(codeSeq.length)]);//random.nextInt(10));
			...
			/*g.drawString(rand, 10 * i + 6 + getRandInt(0, 5),
					25 + getRandInt(0, 5));*/
			//g.drawString(rand, 15 * i + 5, 19 + random.nextInt(8));
			g.drawString(rand, 15 * i + getRandInt(0, 5),  25 + getRandInt(0, 5));
		}
		// 图象生效
		g.dispose();
		return image;
