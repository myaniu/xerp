package com.nutzside.common.validation.annotation;
...
import com.nutzside.common.validation.Errors;
import com.nutzside.common.validation.Validation;
import com.nutzside.common.validation.ValidationUtils;
	...
	public void validate(Object target, Errors errors) {
		...
		for (Field field : fields) {
			...
			try {
				...
				// 验证是否为 email
				if (vals.email() && !ValidationUtils.email(field.getName(), value,
						errMsg, errors)) {
					continue;
				}
				...
				// 验证是否为 qq 号
				if (vals.qq()
						&& !ValidationUtils.qq(field.getName(), value,
								errMsg, errors)) {
					continue;
				}
				...
				// 验证正则表达式
				if (!Strings.isBlank(vals.regex()) && !ValidationUtils.regex(field.getName(), value,
				...
				if (vals.strLen().length > 0
						&& !ValidationUtils.stringLength(field.getName(), value,
								vals.strLen(), errMsg, errors)) {
					continue;
				}
